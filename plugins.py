...
def reloadPlugins(servers, onLoadPlugins, onNewEpisodePlugins,
                  onFinishPlugins, settings):
    modules = []
    ...
    for modulename in modules:
        ...
        for elem in dir(module_obj):
            ...
            if inspect.isclass(obj):
                try:
                    # server plugin
                    if issubclass(obj, plugins_dir.base.serverPlugin):
                        ...
                        if (type(serverName) == type(())):
                            ...
                            servers[serverName] = [obj, ]
                                    ...
                                    plugins_dir.base.abstractAdditionalPlugin):
                        obj = obj(settings)
                        # plugin after load
                        if issubclass(obj.__class__,
                                      plugins_dir.base.onLoadPlugin):
                            onLoadPlugins.append(obj)
                        # plugins on new episode found
                        if issubclass(obj.__class__,
                                      plugins_dir.base.onNewEpisodePlugin):
                            onNewEpisodePlugins.append(obj)
                        # server plugin
                        if issubclass(obj.__class__,
                                      plugins_dir.base.onFinishPlugin):
                            onFinishPlugins.append(obj)
                except:
                    msg = "***Error while loading plugin [{}]***"
                    print msg.format(obj.plugin_name)
    pass

if __name__ == '__main__':
    print "This modul works as a bridge between torrent checker and plugins."
